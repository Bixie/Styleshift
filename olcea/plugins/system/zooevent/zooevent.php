...
class plgSystemZooevent extends JPlugin {
	...
	public function onAfterInitialise() {
		...
		// register event
		$zoo->event->dispatcher->connect('item:init', array('plgSystemZooevent', 'itemInit'));
...
//		$zoo->event->dispatcher->connect('type:assignelements', array('TypeEvent', 'assignelements'));
	...
	public function itemInit($event) {

		$item = $event->getSubject();

		// do whatever you'd like to do
		if ($item->application_id == 1 && !in_array($item->access, [2, 7])) {
