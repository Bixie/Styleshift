...
<div class="items <?php if ($itemstitle) echo 'has-box-title'; ?>">
	...
	<?php if ($itemstitle) : ?>
		<h1 class="uk-article-title"><span><span><?php echo $itemstitle; ?></span></span></h1>
	<?php endif; ?>
	...
	<?php
		...
		while((list($key, $item) = each($this->items)) || ($i % $columns != 0)) {
			if ($i % $columns == 0) echo ($i > 0 ? '</div><div class="uk-grid"  data-uk-grid-margin>' : '<div class="uk-grid">');
			$first = ($i % $columns == 0) ? ' first-item' : null;
			echo '<div class="uk-width-1-'.$columns.$first.'">'.$this->partial('item', compact('item', 'layout')).'</div>';
			$i++;
		}
		echo '</div>';
