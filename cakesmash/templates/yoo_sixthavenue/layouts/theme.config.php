...
foreach ($positions as $position) {
    ...
    $grid_classes[$position][] = "tm-{$position} uk-grid uk-grid-match";
...
$body_classes .= $config->get('tm_dropcap') ? ' tm-dropcap' : '';
...
foreach ($blocks as $block) {
    ...
    $block_classes[$block] = "tm-{$block} tm-block";
    $block_classes[$block] .= $config->get("block.{$block}.block-bg");
    $block_classes[$block] .= $config->get("block.{$block}.block-padding");
    $block_classes[$block] .= $config->get("block.{$block}.block-width") ? ' tm-block-fullwidth' : '';
    $block_classes[$block] .= $config->get("block.{$block}.block-offset") ? ' tm-block-offset' : '';
...
$body_config['menu_hover']  = (int) $config->get('menu_hover', 0);
